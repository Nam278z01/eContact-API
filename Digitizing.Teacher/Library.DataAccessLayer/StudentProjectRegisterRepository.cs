...
{
    public partial class StudentProjectRegisterRepository : IStudentProjectRegisterRepository
    {
        private IDatabaseHelper _dbHelper;
        public StudentProjectRegisterRepository(IDatabaseHelper dbHelper)
        {
            _dbHelper = dbHelper;
        }
        /// <summary>
        ...
        public bool Update(StudentProjectRegisterModel model)
        {
            try
            {
                ...
                var result = _dbHelper.CallToValueWithTransaction("dbo.teacher_student_project_register_update", parameters);
                if ((result != null && !string.IsNullOrEmpty(result.ErrorMessage)) && result.ErrorCode != 0)
                {
                    throw new Exception(result.ErrorMessage);
                }
            ...
            string academic_year, int semester, string class_id, out long total)
        {
            ...
            try
            {
                var parameters = new List<IDbDataParameter>
                {
                    ...
                    _dbHelper.CreateInParameter("@student_project_name", DbType.String,  student_project_name),
                ...
                var result = _dbHelper.CallToList<StudentProjectRegisterSearchModel>("dbo.teacher_student_project_register_search", parameters);
